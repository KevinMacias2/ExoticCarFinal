...
const UpdateCategory= (props) => {
  ...
  const getAllbrandId = async () => {
    ...
    if(response.data){
        setformValue({ ...formValue, 
          ...
          minimumWage: response.data.minimumWage
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost/proyectCar/public/api/category/"+id,
      formData,
      {
        headers: {
      ...
      <Stack gap={1} className="col-md-4 mx-auto shadow-lg p-3 mb-5 mt-4 bg-dark text-white rounded">
        ...
        <Form.Group className="mb-3">
                   ...
                   <Form.Group className="mb-3">
                      ...
                      <Form.Control type="number" placeholder="MinimumWage" required name="minimumWage" value={formValue.minimumWage} onChange={onChange} />
                      <Form.Text className="text-muted"></Form.Text>
        ...
        <Stack className=" mx-auto col-md-8">
          <Link to={`/proyectCar/public/categories/`} className='btn btn-primary'>Return</Link>
          <Toaster />
        </Stack>
        </Stack>
      </Stack>
